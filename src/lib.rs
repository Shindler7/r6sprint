...
pub enum ReadModeLog {
    ...
    Errors,
    /// Считывать только денежные операции.
    Exchanges,
}
...
impl<R: Read> LogIterator<R> {
    ...
    /// Вспомогательный метод фильтрующий пустые строки.
    fn non_empty_lines(line: &IoResult<String>) -> bool {
        !line
            .as_ref()
            .ok()
...
pub fn read_log<R: Read>(input: R, mode: ReadModeLog, request_ids: &[u32]) -> Vec<LogLine> {
    ...
    if request_ids.is_empty() {
        ...
        iter_logs
            .filter(|line| request_ids.contains(&line.request_id))
            .collect()
    }
}
...
App::Journal BuyAsset UserBacket{"user_id":"Alice","backet":Backet{"asset_id":"milk","count":5,},} requestid=10
    ...
    fn test_all() {
        ...
        let sourced = SOURCE1.as_bytes();
        assert_eq!(read_log(sourced, ReadModeLog::All, &request_ids).len(), 1);

        // Многострочный парсинг.
        let sourced = SOURCE.as_bytes();
        let all_parsed = read_log(sourced, ReadModeLog::All, &request_ids);
