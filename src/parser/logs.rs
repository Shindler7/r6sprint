//! Типизированные модели данных для парсеров логов.

use crate::parser::{
    combinators::{
        All, Alt, Delimited, KeyValue, Map, Permutation, Preceded, StripWhitespace, Tag, Unquote,
    },
    constructors::*,
    domain::{Announcements, AuthData, UserBacket, UserCash},
    stdp,
    traits::Parsable,
};
        ...
        preceded(
            ...
            alt4(
                map(
                    ...
                    |authdata| AppLogTraceKind::Connect(Box::from(authdata)),
                ),
                map(
                    preceded(
                        strip_whitespace(tag("SendRequest")),
...
mod tests {
    ...
    fn test_log_kind() {
        ...
        assert_eq!(
            ...
            Ok((
                "".into(),
                LogKind::App(Box::from(AppLogKind::Journal(
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal RegisterAsset {"asset_id": "bayc", "liquidity": 100000000, "user_id": "Steeve",}"#.into()), Ok(("".into(), LogKind::App(Box::from(AppLogKind::Journal(AppLogJournalKind::RegisterAsset { asset_id: "bayc".into(), user_id: "Steeve".into(), liquidity: 100_000_000 }))))));
        assert_eq!(
            LogKind::parser().parse(
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal BuyAsset UserBacket{"user_id": "Steeve", "backet": Backet{"asset_id":"bayc","count":1,},}"#.into()), Ok(("".into(), LogKind::App(Box::from(AppLogKind::Journal(AppLogJournalKind::BuyAsset(UserBacket { user_id: "Steeve".into(), backet: Backet { asset_id: "bayc".into(), count: 1 } })))))));
    }
}
